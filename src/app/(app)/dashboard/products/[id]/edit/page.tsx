...
import { Loader2, ArrowLeft, ImageIcon, FileText } from "lucide-react"
import { z } from "zod"
...
// ─── API ──────────────────────────────────────────────────────────
async function fetchProduct(id: string): Promise<ProductWithImageUrl> {
...
async function updateProduct(id: string, formData: FormData): Promise<void> {
  ...
  if (!res.ok) {
    const body = (await res
      .json()
      .catch(() => ({ error: "更新に失敗しました" }))) as ApiErrorResponse
    throw new Error(extractErrorMessage(body))
  }
}

// ─── ファイルピッカー（new ページと共通の UI） ────────────────────
      ...
      >
        ...
        {hasExisting && !file ? (
          currentUrl ? (
            ...
            <>
              <Icon className="size-7 text-gray-400" aria-hidden="true" />
          ...
          <>
            <Icon className="size-7 text-indigo-500" aria-hidden="true" />
...
export default function EditProductPage() {
  ...
  const mutation = useMutation({
    mutationFn: (fd: FormData) => updateProduct(id, fd),
    onSuccess: () => {
  ...
  // 403 / 404 → 一覧に戻す
  const fetchStatus = (fetchError as (Error & { status?: number }) | null)?.status
  const isAccessDenied = fetchStatus === 403 || fetchStatus === 404
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900">商品を編集する</h1>
        <p className="mt-1 text-sm text-gray-500">
          内容を変更して「保存する」を押してください
        </p>
      </div>
      ...
      {isAccessDenied && (
        <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">
          商品が見つかりませんでした。
          <Link href="/dashboard/products" className="ml-2 underline underline-offset-2">
            一覧に戻る
          </Link>
        </div>
      )}
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            <FieldGroup className="gap-6">
              ...
              <FilePickerField
                ...
                icon={FileText}
                currentName={product.filePath?.split("/").pop() ?? null}
                file={productFile}
